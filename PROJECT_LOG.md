...
- The earlier ignored artifact remains `artifacts/results-catalog-consistency-uat-20260728/`. A fresh isolated UAT after `889a075` created one new persisted permission-request report, confirmed schema `7` and `ReportSynthesis` `2.2`, found zero matching cases, and proved that refresh/history re-entry reuse the same report. The ignored evidence artifact is `artifacts/results-catalog-consistency-uat-889a075-final-20260729-080617/`.
