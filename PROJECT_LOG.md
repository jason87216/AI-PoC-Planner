...
- The earlier ignored artifact remains `artifacts/results-catalog-consistency-uat-20260728/`. A fresh isolated UAT after `8e1df66` created one new persisted permission-request report, confirmed schema `7` and `ReportSynthesis` `2.2`, found zero matching cases, found zero old generic labels, and proved that refresh/history re-entry reuse the same report. The ignored evidence artifact is `artifacts/results-catalog-consistency-uat-8e1df66-final-20260729-123606/`.
- Final verification passed: `620 passed, 6 skipped`, Ruff, format check, and `git diff --check`.
